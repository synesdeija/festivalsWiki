...
module.exports = {
    ...
    getFeed: async(_, res) => { //find all events, sort them by time-descending. event is our model. look at schema on model to see what each event created will have (like createdAt). 
        try {
    ...
    getEvent: async(req, res) => {
        try {
            ...
            const comments = await Comment.find({ events: req.params.id }).sort({ createdAt: "desc" }).lean();
            console.log(comments)
            res.render("event.ejs", { event: event, comments: comments, user: req.user });
        } catch (err) {
            console.log(err);
        }
    },
